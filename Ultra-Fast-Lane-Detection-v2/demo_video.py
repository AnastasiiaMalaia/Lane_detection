...
# Путь к видеофайлу
video_path = '/home/anamaly/Lane_detection/CF_short_compressed_resized.mp4'

# Папка, куда будут сохраняться кадры
output_folder = '/home/anamaly/Lane_detection/video_caps'
...
# Чтение и сохранение каждого кадра
while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
        break
...
if __name__ == "__main__":
    ...
    #     raise NotImplementedError
    imgs_path = os.listdir(output_folder)
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
    print("output."+'avi')
    vout = cv2.VideoWriter('output.avi', fourcc , 1.0, (img_w, img_h))
    for imgname in imgs_path:
        img = cv2.imread(output_folder + '/' + imgname)
        im0 = img.copy()
